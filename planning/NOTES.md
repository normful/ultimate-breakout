...
# Gameplay: Competitive individual players
...
- The leaderboard is displayed outside of the canvas.
- The leaderboard displays colors along with the user's name.
- The leaderboard data is persisted on the server.
- The leaderboard data is reset every week.

## Player names

- A random funny name will be generated by the server for each player and assigned by the server.

## Player colors

- A random color will be assigned to each user by the server.
- Each player's ball and paddle will be drawn using that color.
...
- The game never ends.
- When no users are connected, the bricks are reset.
